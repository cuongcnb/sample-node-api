...
exports.get = (url, params) => {
    ...
    return new Promise((resolve, reject) => {
        let instance = new Graph('GET', url, (err, res) => {
            if (!err) {
                resolve(res);
            } else {
                reject(err)
...
exports.post = function (url, postData, callback) {
    ...
    return new Promise((resolve, reject) => {
        let instance = new Graph('POST', url, postData, (err, res) => {
            if (!err) {
                resolve(res);
            } else {
                reject(err);
...
exports.extendAccessToken = async (params) => {
    let self = this;
    ...
    params.client_secret = this.getAppSecret();

    return await this.get("/oauth/access_token", params);
};

/**
 * Extends the expiration time of page's accessToken
 *
 * @param {object} params containing:
 *   - page_id
 *   - access_token
 */
exports.extendPageAccessToken = async (params) => {
    var self = this;

    const res = await this.extendAccessToken({access_token: params.access_token});

    return await this.get("/" + params.page_id, {
...
/**
 * Sets the app id, used to verify all API calls if provided
 * @param {string} token
 */
